...
def streamlit_config():
    ...
    def setting_bg():
        st.markdown(f""" <style>.stApp {{
                            background:url("https://img.freepik.com/free-photo/abstract-digital-grid-black-background_53876-97647.jpgsss");
    ...
    # title and position
    st.markdown(f'<h1 style="text-align: center; color: #FF5A5F;">Retail Sales Forecast</h1>',
                unsafe_allow_html=True)
    add_vertical_space(1)
    
    st.markdown(f"""<h4 style="text-align: center;">Retail Sales Forecast employs advanced machine learning techniques, prioritizing careful data preprocessing, 
                feature enhancement, and comprehensive algorithm assessment and selection. 
                The streamlined Streamlit application integrates Exploratory Data Analysis (EDA) to find trends, 
                patterns, and data insights. It offers users interactive tools to explore top-performing stores and departments, 
                conduct insightful feature comparisons, and obtain personalized sales forecasts. With a commitment to delivering actionable insights, 
                the project aims to optimize decision-making processes within the dynamic retail landscape.</h4>""", unsafe_allow_html=True)
    add_vertical_space(1)
...
class sql:
    ...
    def create_table():
        ...
        try:
            ...
            elango = psycopg2.connect(host='localhost',
                                    user='postgres',
                                    password='3045',
                                    database='retail_forecast')
            cursor = elango.cursor()
    ...
    def data_migration():
        ...
        try:
            f = pd.read_csv(r'C:\Users\Hxtreme\Desktop\Final project\Datasets\df_sql (1).csv')
            df = pd.DataFrame(f)
    ...
    def predict_weekly_sales():
        ...
        if button:
            with st.spinner(text='Processing...'):
                ...
                with open(r'C:\Users\Hxtreme\Desktop\Final project\model1_markdown.pkl', 'rb') as f:
                    model = pickle.load(f)
...
with st.sidebar:
    ...
    option = option_menu(menu_title='', options=['Migrating to SQL', 'Top Sales', 'Comparison', 'Features', 'Prediction', 'Exit'],
                         icons=['database-fill', 'bar-chart-line', 'globe', 'list-ta3045', 'slash-square', 'sign-turn-right-fill'])
